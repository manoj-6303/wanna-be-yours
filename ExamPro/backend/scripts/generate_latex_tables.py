...
def gen_qs(topic, diff, count):
    ...
    for i in range(count):
        ...
        # Every 5th question will include a LaTeX array (table) to render correctly in KaTeX
        if i % 5 == 0:
            ...
            question_text += "$$\\large\\def\\arraystretch{1.8}\\begin{array}{|c|c|c|}\n"
            question_text += "\\hline\n"
            question_text += "\\quad \\text{Category} \\quad & \\quad x_i \\quad & \\quad f_i \\quad \\\\\n"
            question_text += "\\hline\n"
            question_text += f"\\text{{A}} & {val} & {val + 2} \\\\\n"
            question_text += f"\\text{{B}} & {val + 1} & {val + 3} \\\\\n"
            question_text += f"\\text{{C}} & {val + 2} & {val + 1} \\\\\n"
            question_text += "\\hline\n"
            question_text += "\\end{array}$$\n"
...
print(f'Successfully generated and inserted {len(res.inserted_ids)} Mathematics questions across {len(topics)} chapters (including perfectly formatted KaTeX tables)!')
