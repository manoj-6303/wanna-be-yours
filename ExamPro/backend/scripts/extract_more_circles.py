...
# Paths
TOPIC_DIR = r"D:\lakshya Demo\ExamPro\QuestionBank\JeeMains\Maths\Circles"
TOPIC_NAME = "Circles"
TOPIC_SLUG = "circles"
...
def process_topic_pdfs():
    ...
    # We are specifically targeting PDFs that failed previously
    pdf_files = ['Circle.pdf', 'circles_1.pdf', 'circles_2.pdf', 'circles_3.pdf', 'cqb_jm_jan_2020_math_circle.pdf', 'Circle - JEE Main 2024 April Question Bank - MathonGo.pdf']
    if not pdf_files:
    ...
    image_counters = {"easy": 200, "medium": 200, "hard": 200}
...
4. Distinguish between images in the question vs images in the solution. If the question prompt has a diagram, set `has_question_image` to true. If the explanation has a diagram, set `has_solution_image` to true.
                    ...
                    try:
                        ...
                        if result and result.questions:
                            ...
                            for q in result.questions:
                                ...
                                q_image_filename = None
                                s_image_filename = None
                                if q.has_question_image:
                                    q_image_filename = f"{TOPIC_SLUG}_{diff_lower}_q_{image_counters[diff_lower]}.png"
                                    image_counters[diff_lower] += 1
                                if q.has_solution_image:
                                    s_image_filename = f"{TOPIC_SLUG}_{diff_lower}_s_{image_counters[diff_lower]}.png"
                                    image_counters[diff_lower] += 1
                                ...
                                mongo_doc = {
                                    ...
                                    "question": q.question,
                                    "questionImage": q_image_filename,
                                    "options": [opt.model_dump() for opt in q.options],
                                    "correctAnswer": q.correctAnswer,
                                    "solutionImage": s_image_filename,
                            ...
                            print(f"  No questions extracted from batch")
                        ...
                        if '429' in str(ex) or 'RESOURCE_EXHAUSTED' in str(ex):
                            ...
                            print(f"  Gemini error on batch: {ex}")
                            break
