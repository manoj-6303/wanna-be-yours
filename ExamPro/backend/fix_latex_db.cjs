...
async function run() {
  ...
  for (let q of qs) {
    ...
    if (q.question && typeof q.question === 'string') {
      ...
      if (fixed !== q.question) { q.question = fixed; changed = true; }
    } else if (q.question && q.question.text) {
      const fixed = fixText(q.question.text);
      if (fixed !== q.question.text) { q.question.text = fixed; changed = true; }
    }
    ...
    if (Array.isArray(q.options)) {
      for (let i=0; i<q.options.length; i++) {
        ...
        if (typeof opt === 'string') {
          ...
          if (fixed !== opt) { q.options[i] = fixed; changed = true; }
        } else if (opt && opt.text) {
          const fixed = fixText(opt.text);
          if (fixed !== opt.text) { q.options[i].text = fixed; changed = true; }
        }
      }
    }
    ...
    if (q.explanation) {
      ...
      if (fixed !== q.explanation) { q.explanation = fixed; changed = true; }
    }
    
    if (changed) {
      await Q.updateOne({_id: q._id}, {$set: q});
      updatedCount++;
    }
  }
