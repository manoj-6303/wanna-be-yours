  ...
  for (let q of questions) {
    ...
    if (q.options && Array.isArray(q.options)) {
      const newOptions = q.options.map(opt => {
        ...
        return { ...opt, text, value: (opt.value || text) };
      });
    ...
    if (changed) {
      optionsFixed++;
      await Question.updateOne({ _id: q._id }, { $set: { options: q.options, question: q.question } });
    }
  }

  console.log(`Successfully fixed options/notes in ${optionsFixed} questions. Notes removed: ${notesRemoved}`);
  process.exit(0);
});
