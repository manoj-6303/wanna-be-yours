...
export default function Admin() {
  ...
  const [customTestForm, setCustomTestForm] = useState({
    subject: '', chapter: '', difficulty: 'Medium', examType: 'General', level: 1, passingPercentage: 40, duration: 60, fee: 20
  });
    ...
    if (activeTab === 'questions' && !loading) {
      const fetchQuestions = async () => {
        try {
          ...
          console.error('Failed to fetch grouped questions', error);
        }
      };
      fetchQuestions();
    }
  }, [qSearch, qFilters, activeTab]);
  ...
  const handleImageUpload = async (e, field) => {
    ...
    try {
      ...
      setQuestionFormData(prev => ({ ...prev, [field]: res.data.filename }));
    } catch (error) {
        ...
        <div className="flex-1 bg-white rounded-2xl shadow-sm border border-gray-100 p-8 mb-8">
                    ...
                    <div key={chapterId} className="border border-gray-200 rounded-xl bg-white shadow-sm overflow-hidden">
                      ...
                      >
                        <h3 className="font-bold text-gray-900 text-lg">
                          {chapterGroup._id.subject} - {chapterGroup._id.chapter}
                        </h3>
                              ...
                              <div className="flex-1 pr-6">
                                ...
                                {q.questionImage && (
                                  <div className="mt-3 mb-3 text-sm">
                                    ...
                                    <img 
                                      src={`/images/${q.questionImage}`} 
                                      alt="Question figure" 
                                      className="mt-2 max-w-[50%] rounded border border-gray-200" 
                          ...
                          {visibleCount < chapterGroup.questions.length && (
                            <div className="flex justify-center mt-4">
                              <button 
                                ...
                                className="px-6 py-2 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 rounded-full font-medium transition-colors border border-indigo-200"
                              >
          ...
          {activeTab === 'custom-tests' && (
            <div>
              ...
              <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm mb-8">
                <form onSubmit={handleCreateCustomTest} className="space-y-6">
                  ...
                  <div className="flex justify-between items-center pt-4 border-t border-gray-100">
                    <div className="flex items-center text-indigo-700 font-bold bg-indigo-50 px-4 py-2 rounded-lg border border-indigo-100">
                      <span>{selectedQuestionIds.length} Questions Selected</span>
                    </div>
                    <button type="submit" className="bg-indigo-600 text-white px-6 py-2 rounded-lg font-bold hover:bg-indigo-700 shadow-sm">
              ...
              <h3 className="text-xl font-bold text-gray-900 mb-4 mt-8">Select Questions from Question Bank</h3>
              <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm mb-6 flex flex-wrap gap-4 items-center sticky top-0 z-20">
                <input 
                  type="text" 
                  placeholder="Search questions..." 
                  value={qSearch} 
                ...
                <div className="font-bold text-indigo-700 bg-indigo-50 px-4 py-2 rounded-lg border border-indigo-200 ml-auto">
              ...
              <div className="space-y-4 max-h-[700px] overflow-y-auto pr-2 pb-10">
                {groupedChapters.map((chapterGroup) => {
                  const chapterId = `test-${chapterGroup._id.subject}-${chapterGroup._id.chapter}`;
                  const isExpanded = expandedChapter === chapterId;
                  return (
                    <div key={chapterId} className="border border-gray-200 rounded-xl bg-white shadow-sm overflow-hidden">
                      <div 
                        ...
                        <div className="p-4 bg-white border-t border-gray-100 space-y-3">
                          <div className="flex justify-end mb-2">
                            <button 
                              type="button" 
                              onClick={() => {
                                const allIds = chapterGroup.questions.map(q => q._id);
                              ...
                              className="text-xs bg-indigo-100 text-indigo-700 px-3 py-1 rounded font-bold hover:bg-indigo-200"
                            >
                              Select / Deselect All
                            </button>
                          </div>
                          {chapterGroup.questions.slice(0, visibleCount).map(q => (
                            <div key={q._id} className="flex items-start space-x-3 p-3 bg-gray-50 border border-gray-100 rounded-lg cursor-pointer hover:bg-indigo-50 transition-colors" onClick={() => toggleQuestionSelection(q._id)}>
                              <input type="checkbox" checked={selectedQuestionIds.includes(q._id)} onChange={() => {}} className="mt-1 h-5 w-5 text-indigo-600 border-gray-300 rounded cursor-pointer" />
                              <div className="flex-1">
                                <div className="text-sm font-medium text-gray-900"><MathRenderer text={q.question} /></div>
                                {q.questionImage && (
                                  <img src={`/images/${q.questionImage}`} alt="Question figure" className="mt-2 max-w-[50%] rounded border border-gray-200" onError={(e) => { e.target.style.display = 'none'; }} />
                                )}
                                {q.options && q.options.length >= 4 && (
                                  <div className="mt-2 grid grid-cols-2 gap-2 text-xs text-gray-600">
                                    <div>A: <MathRenderer text={q.options[0]?.value || q.options[0]?.text || q.options[0]} /></div>
                                    <div>B: <MathRenderer text={q.options[1]?.value || q.options[1]?.text || q.options[1]} /></div>
                                    <div>C: <MathRenderer text={q.options[2]?.value || q.options[2]?.text || q.options[2]} /></div>
                                    <div>D: <MathRenderer text={q.options[3]?.value || q.options[3]?.text || q.options[3]} /></div>
                                  </div>
                                )}
                                <div className="mt-2 text-xs text-gray-600 bg-gray-100 p-2 rounded">
                                  <span className="font-semibold text-green-700">Correct: {q.correctAnswer}</span> <br/>
                                  <span className="font-semibold text-gray-700">Explanation:</span> <MathRenderer text={q.explanation} />
                                  {q.solutionImage && (
                                    <img src={`/images/${q.solutionImage}`} alt="Solution figure" className="mt-2 max-w-[50%] rounded border border-gray-200" onError={(e) => { e.target.style.display = 'none'; }} />
                                  )}
                                </div>
                  ...
                  <form id="questionForm" onSubmit={handleSaveQuestion} className="space-y-4">
                    ...
                    <div>
                      <label className="block text-sm font-medium mb-1 flex justify-between">
                        ...
                        <div className="flex items-center text-indigo-600 text-xs cursor-pointer hover:underline">
                           <input type="file" id="qImageUpload" className="hidden" accept="image/*" onChange={(e) => handleImageUpload(e, 'questionImage')} />
                           <label htmlFor="qImageUpload" className="cursor-pointer">📎 Attach Image</label>
                        </div>
                      </label>
                      ...
                      {questionFormData.questionImage && (
                        <div className="mt-2 text-xs text-gray-500 bg-gray-100 p-2 flex justify-between items-center rounded border">
                          <span>Attached: {questionFormData.questionImage}</span>
                          <button type="button" onClick={() => setQuestionFormData({...questionFormData, questionImage: ''})} className="text-red-500">Remove</button>
                        </div>
                      )}
                    </div>
                    <div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-medium mb-1 flex justify-between">
                          ...
                          <div className="flex items-center text-indigo-600 text-xs cursor-pointer hover:underline">
                             <input type="file" id="sImageUpload" className="hidden" accept="image/*" onChange={(e) => handleImageUpload(e, 'solutionImage')} />
                             <label htmlFor="sImageUpload" className="cursor-pointer">📎 Attach Image</label>
                          </div>
                        </label>
                        ...
                        {questionFormData.solutionImage && (
                          <div className="mt-2 text-xs text-gray-500 bg-gray-100 p-2 flex justify-between items-center rounded border">
                            <span>Attached: {questionFormData.solutionImage}</span>
                            <button type="button" onClick={() => setQuestionFormData({...questionFormData, solutionImage: ''})} className="text-red-500">Remove</button>
                          </div>
                        )}
                      </div>
                      <div>
